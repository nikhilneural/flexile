...
export function getDemoCurrentUser(overrides: Partial<CurrentUser> = {}): CurrentUser {
...
export async function getEffectiveCurrentUser(req: Request): Promise<CurrentUser | null> {
  try {
  ...
  try {
    ...
    if (clerkUserId) {
      try {
        ...
        return getDemoCurrentUser({ id: clerkUserId });
      }
    }
  } catch {
    // Ignore error
