...
import { findCompany, getFlexileFeeCents, getRecommendedInvoiceNumber, parseInvoiceRequest } from "./helpers";
...
export async function POST(req: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  const invoiceDateString = parsed.invoiceDate || formatISO(new Date(), { representation: "date" });
  ...
  const invoiceNumber = parsed.invoiceNumber || (await getRecommendedInvoiceNumber(company.id, user.id));

  const compliance = user.userComplianceInfos[0];
  const isBusiness = Boolean(compliance?.businessEntity ?? false);
  const billFrom = isBusiness
  ...
  const invoice = await db.transaction(async (tx) => {
    const [inv] = await tx
      ...
      .values({
        ...
        dueOn: invoiceDateString,
        invoiceType: "services",
        invoiceNumber,
        billFrom,
        billTo: company.name || "Company",
  ...
  return NextResponse.json({ success: true, invoice: { id: invoice.externalId } }, { status: 201 });
}
