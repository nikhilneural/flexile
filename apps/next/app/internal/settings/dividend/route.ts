...
export async function GET(req: Request) {
  ...
  return NextResponse.json({
    minimum_dividend_payment_in_cents: Number(user.minimumDividendPaymentInCents ?? 0n),
...
export async function PATCH(req: Request) {
  ...
  const rawVal = body.user?.minimum_dividend_payment_in_cents ?? body.minimum_dividend_payment_in_cents;
